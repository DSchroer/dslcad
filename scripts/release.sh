...
# Build Linux
(
  cargo build --release --target x86_64-unknown-linux-gnu -F opencasopencascade-sys/builtin -vv
  (cd target/x86_64-unknown-linux-gnu/release/ && zip ../../../linux.zip dslcad)
)

# Build Windows
(
  cargo build --release --target x86_64-pc-windows-gnu -F opencasopencascade-sys/builtin -vv
...
(
  ...
  cargo build --release --target x86_64-apple-darwin -F opencasopencascade-sys/builtin -vv
  (cd target/x86_64-apple-darwin/release/ && zip ../../../macosx.zip dslcad)
)

# Build WASM
(
  CARGO_ARGS="--release -F opencasopencascade-sys/builtin -vv" ./scripts/wasm.sh
